...
export class ManageDonationComponent implements OnInit {
  ...
  toggleItemCollection(donationId: string, itemIndex: number) {

    this.donationService.toggleItemCollection(donationId, itemIndex).subscribe(() => {
      this.donation.items[itemIndex].collect = !this.donation.items[itemIndex].collect;
      this.countCollections();
    }, (error) => {
  ...
  {
    // Available when the currect status is 'Awaiting Collection'
    alertify.error('Feature coming soon!');
  }
